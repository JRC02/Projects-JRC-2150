class subject
{
	...
	void modify()
	{
		...
		cin>>nsubcode;
		cout<<"No of lectures: ";
		cin>>n_no_of_lectures;

		subcode=nsubcode;
		strcpy(subname,nsubname);
		no_of_lectures=n_no_of_lectures;
	}*/
...
{       subject sub;
	...
	gets(proxy);
	fstream filin;
	filin.open("C:\\tt\\Subjects.dat", ios::binary|ios::in|ios::out);
	filin.seekg(0);
	...
	{       pos=filin.tellg();
		filin.read((char*)&sub,sizeof(sub));
		if (filin.eof()) break;
		if(strcmpi(sub.getsubname(),proxy)==0)
		{       sub.modify();
			filin.seekg(pos);
			filin.write((char*)&sub,sizeof(sub));
			found='y';
			cout<<"\nSubject updated"<<endl;
			break;
		}
...
//This should also list the classes to which the subject is linked
{	subject sub, s;
	int sid;
