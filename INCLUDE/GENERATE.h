...
classentity clsmap[20];  //Total 20 classes
...
struct classdiv
	{
		...
		struct
		{
			...
			busy[5]; //5 days per week
		}
		busy[5]; //5 lectures per day
	};
...
struct steacher
	...
	steacher tchr[20]; //Total 20 teachers
	...
	struct
		{
			...
			fsubj[5];  //day of the week
		}
		fsubj[5];	//lecture of the day
	};
...
struct ftchr
	{
		struct
		{
			...
			ftchr[5]; //day of the week
		}
		ftchr[5]; 	//lecture of the day
	};
	...
	struct
		{
			...
			tmpsubj[5];  //day of the week
		}
		tmpsubj[5];	//lecture of the day
	}
	tmpsubj[20];  //class-div
...
struct strtchr
	{
		struct
		{
			...
			tmptchr[5]; //day of the week
		}
		tmptchr[5]; //lecture of the day
	}
	tmptchr[20]; //class-div
		...
		do {
			...
			if (!cfound) {
				...
				if (ccount<20) {
					...
					allgood=0;
					cout<<"\n\tWARNING: The application is already configured for 20 classes. It cannot accommodate additional classes."<<endl;
				}
