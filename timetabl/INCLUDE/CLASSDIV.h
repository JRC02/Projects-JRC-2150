...
void Class::addrecords()
{
	...
	if (c.getccount() < 20) {
		...
		cout<<"\n\tWARNING: Maximum limit of 20 classes reached. Cannot add."<<endl;
	}
}
...
{     
	...
	if (filin.bad() || filin.fail())  {
		cout<<"\n\tClass data does not exist."<<endl;
		filin.clear();
	} else {
		
		filout.open("C:\\tt\\temp.dat", ios::binary|ios::out);

		cout<<"\n\tEnter class ID to be modified: ";
		cin>>idno;
		
		cout<<".----------------------------------------------------------------------------."<<endl;
		cout<<"|                        1.4 Modify class details                            |"<<endl;
		cout<<"|----------------------------------------------------------------------------|"<<endl;
		...
		while(1) {
			...
			{   
				...
				if (c.modify()==-1) {
					...
					filout.write((char*)&c,sizeof(c));
					cout<<"\n\tClass record modified successfully."<<endl;
					modified=1;
				}
			}
			else
		...
		if(!found)
			cout<<"\tThe Class record not found."<<endl;
			...
			{	
				...
				if (c.classid==cid) {
					...
					for (int i=0; i<c.total_subjects; i++) {
						...
						cout<<"   "<<c.subjects[i].subjcode<<"   "<<prns<<"   \t\t"<<c.subjects[i].numlectures;
